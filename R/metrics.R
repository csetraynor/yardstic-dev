...
metrics.data.frame  <-
  function(data, truth, estimate, ..., options = list(), na.rm = TRUE) {
    ...
    #### Is survival?
    if('time' %in% colnames(vars) & 'status' %in% colnames(vars)){
      vars$truth <- with(vars, Surv(time,status))}

    is_survival <- survival::is.Surv(data[[ vars$truth ]])
    ...
    if(is_survival){
      ...
      res <- dplyr::tibble(
        tdROC = tdROC_calc(data[[ vars$truth ]], data[[ vars$estimate ]])
        # rsq = rsq_calc(data[[ vars$truth ]], data[[ vars$estimate ]]),
        # mae = mae_calc(data[[ vars$truth ]], data[[ vars$estimate ]])
      )
